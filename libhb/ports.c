...
/************************************************************************
 * CPU topology detection (NUMA nodes / CCD - L3 clusters) and
 * CCD/NUMA-aware thread affinity.
 ***********************************************************************/
static hb_cpu_topology_t hb_cpu_topology;
static int hb_cpu_topology_done   = 0;
static int hb_cpu_affinity_enabled = -1; /* -1 = auto/uninitialised */

#if defined(SYS_LINUX)
static int hb_read_sysfs_int(const char *path, int *out)
{
    FILE *f = fopen(path, "r");
    if (f == NULL)
    {
...
static void init_cpu_topology(void)
{
    ...
    hb_cpu_topology_t t;
    memset(&t, 0, sizeof(t));
    t.logical_cpus   = hb_get_cpu_count();
    t.physical_cores = 0;
    t.numa_nodes     = 1;
    t.ccd_count      = 1;
...
#if defined(SYS_CYGWIN) || defined(SYS_MINGW)
    {
        ...
        if (len > 0)
        {
            ...
            {
                int numa = 0, l3 = 0, cores = 0;
                uint8_t *p = buf, *pend = buf + len;
                while (p < pend)
                {
                ...
                if (numa  > 0) t.numa_nodes     = numa;
                if (cores > 0) t.physical_cores = cores;
                if (l3    > 0) t.ccd_count      = l3;
            }
            free(buf);
        }
    }
#elif defined(SYS_LINUX)
    {
        /* Count NUMA nodes via /sys/devices/system/node/nodeN */
        int nodes = 0;
        for (int i = 0; i < 256; i++)
        {
            char path[128];
        ...
        /* Count distinct L3 cache ids: on AMD Zen each shared L3 maps
         * to exactly one CCD (core complex die). */
        int seen[512];
        int seen_count = 0;
        for (int cpu = 0; cpu < t.logical_cpus && cpu < 512; cpu++)
        {
            char path[160];
            int id = -1;
            snprintf(path, sizeof(path),
                "/sys/devices/system/cpu/cpu%d/cache/index3/id", cpu);
            if (hb_read_sysfs_int(path, &id) != 0)
            {
                continue;
            }
            int dup = 0;
        ...
        if (seen_count > 0)
        {
            t.ccd_count = seen_count;
        }
    }
#endif

    if (t.numa_nodes < 1) t.numa_nodes = 1;
    if (t.ccd_count  < 1) t.ccd_count  = 1;
    t.cpus_per_ccd = t.logical_cpus / t.ccd_count;
    if (t.cpus_per_ccd < 1) t.cpus_per_ccd = 1;

    hb_cpu_topology      = t;
    hb_cpu_topology_done = 1;
}

hb_cpu_topology_t hb_get_cpu_topology(void)
{
    init_cpu_topology();
    return hb_cpu_topology;
}

void hb_cpu_set_affinity_enabled(int enabled)
{
    hb_cpu_affinity_enabled = enabled ? 1 : 0;
}

int hb_cpu_get_affinity_enabled(void)
{
    if (hb_cpu_affinity_enabled < 0)
    {
        hb_cpu_topology_t t = hb_get_cpu_topology();
        /* Auto-enable only when there is more than one domain to pin to. */
        hb_cpu_affinity_enabled = (t.ccd_count > 1 || t.numa_nodes > 1) ? 1 : 0;
    }
    return hb_cpu_affinity_enabled;
}

static int hb_cpu_effective_domains(const hb_cpu_topology_t *t)
{
    if (t->ccd_count > 1)
    {
        return t->ccd_count;
...
{
    if (domain_count <= 1 || !hb_cpu_get_affinity_enabled())
    {
        return;
    }

    hb_cpu_topology_t t = hb_get_cpu_topology();
    int domains = hb_cpu_effective_domains(&t);
    if (domains <= 1)
    {
        return;
    }

    int real_domain = ((domain % domains) + domains) % domains;
...
#if defined(SYS_CYGWIN) || defined(SYS_MINGW)
    {
        ...
        if (group_count > 1)
        {
            /* On large Windows hosts each processor group is its own
             * affinity island (<= 64 logical CPUs); pin to one group. */
            WORD  group = (WORD)(real_domain % group_count);
            DWORD cpus  = GetActiveProcessorCount(group);
            GROUP_AFFINITY ga;
            memset(&ga, 0, sizeof(ga));
            ga.Group = group;
            ga.Mask  = (cpus >= 64) ? ~(KAFFINITY)0
                                    : (((KAFFINITY)1 << cpus) - 1);
            SetThreadGroupAffinity(GetCurrentThread(), &ga, NULL);
        }
        else
        {
            int width = t.logical_cpus / domains;
            if (width < 1) width = 1;
            int start = real_domain * width;
            DWORD_PTR mask = 0;
            for (int i = start; i < start + width && i < 64; i++)
            {
                mask |= ((DWORD_PTR)1 << i);
            }
            if (mask != 0)
...
#elif defined(SYS_LINUX)
    {
        ...
        cpu_set_t set;
        CPU_ZERO(&set);
        for (int i = start; i < start + width && i < t.logical_cpus; i++)
        {
            CPU_SET(i, &set);
        }
